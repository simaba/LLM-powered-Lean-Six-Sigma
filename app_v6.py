...
def _priority_chart(df: pd.DataFrame) -> alt.Chart:
    return (
        ...
        .properties(title="Improvement prioritization matrix", height=350)
    )
...
st.caption("A structured project-improvement dashboard for operational and executive audiences.")
...
st.markdown('<div class="hero-card"><h3 style="margin:0;">From vague problems to structured improvement action</h3><div class="small-muted" style="color:#dbeafe;">Clarify the problem, surface likely causes, prioritize actions, and build a control model that can actually be operated.</div></div>', unsafe_allow_html=True)
...
if generate:
    ...
    st.markdown("## 5. Action portfolio")
    st.dataframe(action_df, use_container_width=True, hide_index=True)
    for insight in build_operational_insights(result, action_df):
        st.write(f"- {insight}")
    ...
    st.markdown("## 9. Status narrative")
    st.success(build_status_narrative(result, edited_action_df))
    ...
    payload = {
        ...
        "executive_insights": build_executive_insights(result, edited_action_df, edited_control_df),
    }
    ...
    with e2:
        if st.button("Export reviewed package", use_container_width=True):
            exported = export_review_package(project_data["project_name"], payload, edited_action_df, edited_control_df)
